...
def build_query(data_path, w2v_path, vocab_path, k):
    ...
    for i in range(len(disease_field_list)):
        ...
        # 查询扩展(含词干还原和去停用词操作)
        query_tmp_list.append(clean_data(query_extension(disease_list, w2v_model, vocab, k)))
        query_tmp_list.append(clean_data(query_extension(gene_list, w2v_model, vocab, k)))
        query_tmp_list.append(clean_data(query_extension(demographic_list, w2v_model, vocab, k)))
        query_tmp_list.append(clean_data((query_extension(other_list, w2v_model, vocab, k)))
